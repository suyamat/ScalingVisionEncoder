...
from models.InternImage.classification.ops_dcnv3 import modules as opsm
...
class InternImageBlock(nn.Module):
    ...
    def forward(self, x, return_wo_downsample=False, extract_layer=None):
...
class InternImage(nn.Module):
    ...
    def forward_features_seq_out(self, x):
        ...
        for i, level in enumerate(self.levels):
            if i==0:
                if self.extract_layer<2: #0~1
                    x, _ = level(x, return_wo_downsample=True, extract_layer=self.extract_layer)
                    break
                else:
                    x, _ = level(x, return_wo_downsample=True, extract_layer=None)
            elif i==1:
                if 2<=self.extract_layer<4: #2~3
                    x, _ = level(x, return_wo_downsample=True, extract_layer=self.extract_layer-2)
                    break
                else:
                    x, _ = level(x, return_wo_downsample=True, extract_layer=None)
            elif i==2:
                if 4<=self.extract_layer<52:
                    x, _ = level(x, return_wo_downsample=True, extract_layer=self.extract_layer-4)
                    break
                else:
                    x, _ = level(x, return_wo_downsample=True, extract_layer=None)
            else: #i==3
                if 52<=self.extract_layer<56:
                    x, _ = level(x, return_wo_downsample=True, extract_layer=self.extract_layer-52)
                    break
                else:
                    x, _ = level(x, return_wo_downsample=True, extract_layer=None)

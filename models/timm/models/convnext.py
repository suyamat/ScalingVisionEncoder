...
from models.timm.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, OPENAI_CLIP_MEAN, OPENAI_CLIP_STD
from models.timm.layers import trunc_normal_, AvgPool2dSame, DropPath, Mlp, GlobalResponseNormMlp, \
    LayerNorm2d, LayerNorm, create_conv2d, get_act_layer, make_divisible, to_ntuple
from models.timm.layers import NormMlpClassifierHead, ClassifierHead
...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        self.extracting_depth = extracting_depth
    ...
    def forward_features(self, x):
        x = self.stem(x)
        if self.extracting_depth==0:
            return x
        
        for i in range(3):
            x = self.stages[0].blocks[i](x)
            if self.extracting_depth==(i+1):
                return x
            
        x = self.stages[1].downsample(x)
        for i in range(4):
            x = self.stages[1].blocks[i](x)
            if self.extracting_depth==(i+4):
                return x
            
        x = self.stages[2].downsample(x)
        for i in range(30):
            x = self.stages[2].blocks[i](x)
            if self.extracting_depth==(i+8):
                return x
        
        x = self.stages[3].downsample(x)
        for i in range(3):
            x = self.stages[3].blocks[i](x)
            if self.extracting_depth==(i+38):
                return x
